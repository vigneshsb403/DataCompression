...
@register_model
def rd_ablation_c64_l10(lmb_range=(4,2048), pretrained=False):
    cfg = dict()
    ...
    cfg['dec_blocks'] = [
        ...
        *[lib.LatentVariableBlock(dec_dims[0], z_dims[0], _emb_dim, enc_width=enc_dims[-1]) for _ in range(1)],
        common.patch_upsample(dec_dims[0], dec_dims[1], rate=2),
        # 2x2
        *[lib.LatentVariableBlock(dec_dims[1], z_dims[1], _emb_dim, enc_width=enc_dims[-2]) for _ in range(2)],
        common.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        *[lib.LatentVariableBlock(dec_dims[2], z_dims[2], _emb_dim, enc_width=enc_dims[-3]) for _ in range(2)],
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        *[lib.LatentVariableBlock(dec_dims[3], z_dims[3], _emb_dim, enc_width=enc_dims[-4]) for _ in range(2)],
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[lib.LatentVariableBlock(dec_dims[4], z_dims[4], _emb_dim, enc_width=enc_dims[-5]) for _ in range(3)],
        common.patch_upsample(dec_dims[4], im_channels, rate=4)
    ]
...
@register_model
def rd_ablation_c64_l15(lmb_range=(4,2048), pretrained=False):
    cfg = dict()
    ...
    cfg['dec_blocks'] = [
        ...
        *[lib.LatentVariableBlock(dec_dims[1], z_dims[1], _emb_dim, enc_width=enc_dims[-2]) for _ in range(2)],
        common.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        *[lib.LatentVariableBlock(dec_dims[2], z_dims[2], _emb_dim, enc_width=enc_dims[-3]) for _ in range(3)],
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        *[lib.LatentVariableBlock(dec_dims[3], z_dims[3], _emb_dim, enc_width=enc_dims[-4]) for _ in range(4)],
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[lib.LatentVariableBlock(dec_dims[4], z_dims[4], _emb_dim, enc_width=enc_dims[-5]) for _ in range(5)],
        common.patch_upsample(dec_dims[4], im_channels, rate=4)
    ]
...
@register_model
def rd_ablation_c96_l15(lmb_range=(4,2048), pretrained=False):
    cfg = dict()
...
@register_model
def rd_ablation_c128_l10(lmb_range=(4,2048), pretrained=False):
    cfg = dict()
    ...
    cfg['dec_blocks'] = [
        ...
        *[lib.LatentVariableBlock(dec_dims[2], z_dims[2], _emb_dim, enc_width=enc_dims[-3]) for _ in range(2)],
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        *[lib.LatentVariableBlock(dec_dims[3], z_dims[3], _emb_dim, enc_width=enc_dims[-4]) for _ in range(3)],
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[lib.LatentVariableBlock(dec_dims[4], z_dims[4], _emb_dim, enc_width=enc_dims[-5]) for _ in range(3)],
        common.patch_upsample(dec_dims[4], im_channels, rate=4)
...
@register_model
def rd_ablation_base_nosmooth(lmb_range=(4,2048), pretrained=False):
    cfg = dict()
    ...
    cfg['dec_blocks'] = [
        ...
        *[lib.LatentVariableBlockOld(dec_dims[0], z_dims[0], _emb_dim, enc_width=enc_dims[-1]) for _ in range(1)],
        common.patch_upsample(dec_dims[0], dec_dims[1], rate=2),
        # 2x2
        *[lib.LatentVariableBlockOld(dec_dims[1], z_dims[1], _emb_dim, enc_width=enc_dims[-2]) for _ in range(2)],
        common.patch_upsample(dec_dims[1], dec_dims[2], rate=2),
        # 4x4
        *[lib.LatentVariableBlockOld(dec_dims[2], z_dims[2], _emb_dim, enc_width=enc_dims[-3]) for _ in range(3)],
        common.patch_upsample(dec_dims[2], dec_dims[3], rate=2),
        # 8x8
        *[lib.LatentVariableBlockOld(dec_dims[3], z_dims[3], _emb_dim, enc_width=enc_dims[-4]) for _ in range(4)],
        common.patch_upsample(dec_dims[3], dec_dims[4], rate=2),
        # 16x16
        *[lib.LatentVariableBlockOld(dec_dims[4], z_dims[4], _emb_dim, enc_width=enc_dims[-5]) for _ in range(5)],
        common.patch_upsample(dec_dims[4], im_channels, rate=4)
    ]
